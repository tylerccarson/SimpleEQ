...
juce::String RotarySliderWithLabels::getDisplayString() const
{
    ...
    {
        jassertfalse; // this shouldn't happen, no other present audio parameter type, but just in case
    }
...
rightPathProducer(audioProcessor.rightChannelFifo)
{
    ...
    updateChain(); // draw initial curve from stored parameters
...
void PathProducer::process(juce::Rectangle<float> fftBounds, double sampleRate)
{
    ...
    // and only display the most recent
...
void ResponseCurveComponent::paint (juce::Graphics& g)
{
    ...
    auto transform = AffineTransform().translation(responseArea.getX(), responseArea.getY());
    leftChannelFFTPath.applyTransform(transform);
    g.setColour(Colours::skyblue);
    g.strokePath(leftChannelFFTPath, PathStrokeType(1.f));
    
    auto rightChannelFFTPath = rightPathProducer.getPath();
    rightChannelFFTPath.applyTransform(transform);
